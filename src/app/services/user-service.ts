...
export class UserService {
  // Ajusta la URL base según corresponda en tu entorno de desarrollo
  ...
  // POST /private/user (Creación de usuario por Admin o Recepcionista)
  createUser(request: any): Observable<UserDtoResponse> {
    return this.http.post<UserDtoResponse>(this.apiUrl, request);
  }

  // GET /private/user/{id}
  getById(id: number): Observable<UserDtoResponse> {
    return this.http.get<UserDtoResponse>(`${this.apiUrl}/${id}`);
  }

  // GET /private/user/dni/{dni}
  getByDni(dni: string): Observable<UserDtoResponse> {
    return this.http.get<UserDtoResponse>(`${this.apiUrl}/dni/${dni}`);
  }
  ...
  updateUser(id: number, request: UserDtoRequest): Observable<UserDtoResponse> {
    return this.http.put<UserDtoResponse>(`${this.apiUrl}/${id}`, request);
  }
